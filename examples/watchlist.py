...
class Watchlist(object):
    ...
    apply technical indicators with a Polars TI Strategy.

    Default Strategy: polars_ti.CommonStrategy
    ...
    - Polars TI (pip install polars_ti)
    ...
    def _plot(self, df, mas: bool = True, constants: bool = False, **kwargs) -> None:
        ...
        if constants:
            ...
            df.ti.constants(True, chart_lines)  # Adding the constants for the charts
            df.ti.constants(
        ...
        if ti.Imports["matplotlib"]:
            _exchange = kwargs.pop("exchange", "NYSE")
            _time = ti.get_time(_exchange, to_string=True)
            ...
            if mas:
                # df.ti.strategy(self.strategy, append=True)
    ...
    def load(
        ...
        **kwargs,
        ...
        if kwargs.pop("analyze", True):
            if self.debug:
                print(f"[+] TI[{len(self.strategy.ti)}]: {self.strategy.name}")
            df.ti.strategy(self.strategy, timed=self.timed, **kwargs)
    ...
    @property
    def strategy(self) -> ti.Strategy:
        """Sets a valid Strategy. Default: polars_ti.CommonStrategy"""
        return self._strategy

    @strategy.setter
    def strategy(self, value: ti.Strategy) -> None:
        if value is not None and isinstance(value, ti.Strategy):
            self._strategy = value
        else:
            self._strategy = ti.CommonStrategy
    ...
    def indicators(self, *args, **kwargs) -> any:
        """Returns the list of indicators that are available with Polars TI."""
        pd.DataFrame().ti.indicators(*args, **kwargs)

    def __repr__(self) -> str:
        s = f"Watch(name='{self.name}', ds_name='{self.ds_name}', tickers[{len(self.tickers)}]='{', '.join(self.tickers)}', tf='{self.tf}', strategy[{self.strategy.total_ti()}]='{self.strategy.name}'"
