...
def load(**kwargs: DictLike):
    ...
    print(f"\n{TEST} Polars TI on {datetime.datetime.now()}")
...
_tdpy = polars_ti.RATE["TRADING_DAYS_PER_YEAR"]
...
sample_data = load(
    ...
    n=[-2 * _tdpy, -_tdpy, -200, -89, 0, 89, 200, _tdpy, 2 * _tdpy][4],
    verbose=VERBOSE,
)
...
# _df = DataFrame()
# tickers =["SQ", "PLTR"]
# data = {t:_df.ti.ticker(t, period="1y", timed=True) for t in tickers if len(t) > 1}
