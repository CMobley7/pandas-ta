...
def test_nb_prenan(array, n, result):
    np.testing.assert_array_equal(ti.nb_prenan(array, n), result)
...
def test_nb_prepend(array, n, result):
    np.testing.assert_array_equal(ti.nb_prepend(array, n), result)


@mark.parametrize("array,n,fn,result", [(np.ones(5), 2, None, np.ones(5))])
def test_nb_rolling(array, n, fn, result):
    np.testing.assert_array_equal(ti.nb_rolling(array, n, fn), result)
...
def test_nb_shift(array, n, result):
    np.testing.assert_array_equal(ti.nb_shift(array, n), result)
