...
def test_above():
    result = ti.utils.above(crosseddf["a"], crosseddf["zero"])
    assert isinstance(result, Series)
    assert result.name == "a_A_zero"
    np.testing.assert_array_equal(result, crosseddf["c"])

    result = ti.utils.above(crosseddf["zero"], crosseddf["a"])
    assert isinstance(result, Series)
...
def test_above_value():
    result = ti.utils.above_value(crosseddf["a"], 0)
    assert isinstance(result, Series)
    assert result.name == "a_A_0"
    np.testing.assert_array_equal(result, crosseddf["c"])

    result = ti.utils.above_value(crosseddf["a"], crosseddf["zero"])
    assert result is None


def test_add_df_prefix(df):
    result = df.ti.hl2(prefix="pre")
    assert result.name == "pre_HL2"

    result = df.ti.hl2(suffix="suf")
    assert result.name == "HL2_suf"

    result = df.ti.hl2(prefix="pre", suffix="suf")
    assert result.name == "pre_HL2_suf"

    result = df.ti.hl2(prefix=1, suffix=2)
    assert result.name == "1_HL2_2"

    result = df.ti.macd(prefix="pre", suffix="suf")
    for col in result.columns:
        assert col.startswith("pre_") and col.endswith("_suf")


def test_below():
    result = ti.utils.below(crosseddf["zero"], crosseddf["a"])
    assert isinstance(result, Series)
...
def test_below_value():
    result = ti.utils.below_value(crosseddf["a"], 0)
    assert isinstance(result, Series)
    assert result.name == "a_B_0"
    np.testing.assert_array_equal(result, crosseddf["b"])

    result = ti.utils.below_value(crosseddf["a"], crosseddf["zero"])
    assert result is None


def test_combination():
    assert ti.utils.combination() != None

    assert ti.utils.combination() == 1
    assert ti.utils.combination(r=-1) == 1

    assert ti.utils.combination(n=10, r=4, repetition=False) == 210
    assert ti.utils.combination(n=10, r=4, repetition=True) == 715


def test_cross_above():
    result = ti.utils.cross(crosseddf["a"], crosseddf["b"])
    assert isinstance(result, Series)
    np.testing.assert_array_equal(result, crosseddf["crossed"])

    result = ti.utils.cross(crosseddf["a"], crosseddf["b"], above=True)
    assert isinstance(result, Series)
    np.testing.assert_array_equal(result, crosseddf["crossed"])

    result = ti.utils.cross(crosseddf["a"], crosseddf["b"], equal=False)
    assert isinstance(result, Series)
    np.testing.assert_array_equal(result, crosseddf["crossed"])


def test_cross_below():
    result = ti.utils.cross(crosseddf["b"], crosseddf["a"], above=False)
    assert isinstance(result, Series)
    np.testing.assert_array_equal(result, crosseddf["crossed"])

    result = ti.utils.cross(crosseddf["b"], crosseddf["a"], above=False, equal=False)
    assert isinstance(result, Series)
    np.testing.assert_array_equal(result, crosseddf["crossed"])

    # result = ti.utils.cross(crosseddf["a"], crosseddf["b"], above=False)
    # assert result[0] == 0


def test_datetime_ordered(df):
    # Test if datetime64 index and ordered
    result = df.ti.datetime_ordered()
    assert result is True

    # Test if not ordered
    original = df.copy()
    reversal = original.ti.reverse()
    result = reversal.ti.datetime_ordered()
...
def test_geometric_mean(df):
    returns = ti.percent_return(df.close)
    result = ti.utils.geometric_mean(returns)
    assert isinstance(result, (float, int))

    result = ti.utils.geometric_mean(Series([12, 14, 11, 8]))
    assert isinstance(result, float)

    result = ti.utils.geometric_mean(Series([100, 50, 0, 25, 0, 60]))
    assert isinstance(result, float)

    series = Series([0, 1, 2, 3])
    result = ti.utils.geometric_mean(series)
    assert isinstance(result, float)
...
def test_log_geometric_mean(df):
    returns = ti.percent_return(df.close)
    result = ti.utils.log_geometric_mean(returns)
    assert isinstance(result, float)

    result = ti.utils.log_geometric_mean(Series([12, 14, 11, 8]))
    assert isinstance(result, float)

    result = ti.utils.log_geometric_mean(Series([100, 50, 0, 25, 0, 60]))
    assert isinstance(result, float)

    series = Series([0, 1, 2, 3])
    result = ti.utils.log_geometric_mean(series)
    assert isinstance(result, float)
...
def test_pascals_triangle():
    assert ti.utils.pascals_triangle(inverse=True) is None
...
def test_pascals_triangle_n(value):
    array = ti.utils.pascals_triangle(n=value)
...
def test_get_time_(value):
    print(f"{value=}")
    result = ti.utils.get_time(value, to_string=True)
...
def test_hpoly(array, degree, result):
    assert ti.utils.hpoly(array, degree) == result


@mark.parametrize("value,result", [(-0.01, np.nan), (1.01, np.nan)])
def test_inv_norm_isnan(value, result):
    np.testing.assert_equal(ti.utils.inv_norm(value), result)
...
def test_inv_norm_value(value, result):
    assert ti.utils.inv_norm(value) == result
...
def test_tal_ma(value, result):
    assert ti.utils.tal_ma(value) == result


def test_to_utc(df):
    result = ti.utils.to_utc(df)
...
def test_version():
    assert isinstance(ti.version, str)
    print(f"{ti.version=}")
...
def test_v_drift_type():
    ...
    for _ in _instances:
        assert isinstance(ti.utils.v_drift(_), int)
...
def test_v_drift_value(value, result):
    assert ti.utils.v_drift(value) == result
...
def test_v_lowerbound_type(value, strict):
    assert isinstance(ti.v_lowerbound(value, strict=strict), (float, int))
...
def test_v_lowerbound_value_strict(value, result):
    assert ti.utils.v_lowerbound(value) == result
...
def test_v_offset_value(value, result):
    assert ti.utils.v_offset(value) == result


@mark.parametrize("value", [None, "", [], {}, -1.1, -1, 0.0, 0, 0.1, 1.0, 1])
@mark.parametrize("strict", [False, True])
def test_v_upperbound_value_strict(value, strict):
    assert isinstance(ti.utils.v_upperbound(value, strict=strict), (float, int))
...
def test_zero(value, result):
    assert ti.utils.zero(value) == result
...
def test_indicator_speed_talib_verbose(df, talib, verbose):
    resultdf = ti.speed_test(df, talib=talib, verbose=verbose)
